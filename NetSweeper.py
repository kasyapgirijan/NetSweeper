    ...
    def udp_scan(ip, eth):
        ...
        udp_command = f"nmap --top-ports=10 -Pn -n -sUV -T4 -e {eth} {ip} -oA {UDP_PATH}/udp_scan_{ip}"
    ...
    def tcp_scan(ip, eth):
        ...
        tcp_command = f"nmap --top-ports=100 -Pn -n -sSV -A -T4 -e {eth} {ip} -oA {TCP_PATH}/tcp_scan_{ip}"
    ...
    def perform_scans(alive_hosts, eth, args):
        results = []
        for ip, ports in alive_hosts.items():
            ports_str = ','.join(ports) if not args.full else "-"
            if args.tcp and args.udp:
                ...
                scan_type = '-sS'  # default to TCP if none specified

            nmap_command = f"nmap {scan_type} -p{ports_str} -Pn -T4 -e {eth} {ip} -oA {TCP_PATH}/{ip}"
            if args.verbose:
                print(f"Running detailed Nmap scan for {ip} with command: {nmap_command}")
            run_command(nmap_command)
            results.extend(parse_nmap_output(f"{TCP_PATH}/{ip}.nmap"))
        return results
    ...
    def main():
        ...
        if args.full:
            print("Performing a full port scan...")

        # Assume masscan is used only for quick discovery or initial scan
        masscan_output = masscan_sweep(eth, args.rate, scope, Path(args.output))
        alive_hosts = parse_masscan_output(masscan_output)
        detailed_results = perform_scans(alive_hosts, eth, args)

        # Export results based on selected formats
        export_results(detailed_results, Path(args.output))
        if args.html:
            export_to_html(detailed_results, Path(args.output) / "detailed_scan_results.html")
        ...
        parser.add_argument("--tcp", action="store_true", help="Perform a TCP scan")
