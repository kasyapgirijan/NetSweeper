...
def udp_scan(ip, eth):
    ...
    udp_command = f"nmap --top-ports=10 -Pn -n -sUV -T3 -e {eth} {ip} -oA {UDP_PATH}/udp_scan_{ip}"
...
def tcp_scan(ip, eth):
    ...
    tcp_command = f"nmap --top-ports=100 -Pn -n -sSV -A -T3 -e {eth} {ip} -oA {TCP_PATH}/tcp_scan_{ip}"
